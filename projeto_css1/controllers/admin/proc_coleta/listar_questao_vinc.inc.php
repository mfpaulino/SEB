...
while($row_questao = $con_questao->fetch_assoc()){
	...
	$con_proc_coleta_vinc = $mysqli->query("SELECT id_proc_coleta_vinc FROM adm_questoes WHERE id_questao = '$id_questao'");
	$row_proc_coleta_vinc = $con_proc_coleta_vinc->fetch_array();
	$lista_id_proc_coleta = unserialize($row_proc_coleta_vinc[0]);

	$checked="";
	if($row_proc_coleta_vinc[0] <> ""){//para evitar que a função in_array seja executada para um array vazio
		if (in_array($id_proc_coleta, $lista_id_proc_coleta)) {
			$checked="checked";
		}
	}
	?>

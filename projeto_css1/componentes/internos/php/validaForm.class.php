...
class validaForm {
    ...
    protected function set_messages_default(){
        $this->_messages = array(
            ...
            'min_value'      => 'O valor do campo %s deve ser no mínimo %s ! ',
            'max_value'      => 'O valor do campo %s deve ser no máximo %s !',
            ...
            'is_equals'      => 'O campo %s deve ser igual ao campo %s !',
            'is_not_equals'  => 'O campo %s não deve ser igual ao campo %s !',
     ...
     * @return Data_Validator The self instance
     */
    /* codigo abaixo comentado por Cap Paulino e substituido pelo codigo mais abaixo
    ...
    */
    public function min_length($length){
        $verify = (strlen($this->_data['value']) >= $length);
        if (!$verify){
            $this->set_error(sprintf($this->_messages['min_length'], $this->_data['name'], $length));
        }
        return $this;
	...
	*/
    public function min_value($value){
        $verify = (!is_numeric($this->_data['value']) || $this->_data['value'] >= $value);
        if (!$verify){
            $this->set_error(sprintf($this->_messages['min_value'], $this->_data['name'], $value));
        }
        return $this;
	...
	*/
     ...
     * @return Data_Validator The self instance
     */
    public function is_equals($value, $identical = false, $campo = ""){
        $verify = ($identical === true ? $this->_data['value'] === $value : strtolower($this->_data['value']) == strtolower($value));
        if(!$verify){
			if($campo == ""){
				$this->set_error(sprintf($this->_messages['is_equals'], $this->_data['name'], $value));//mostra o valor do campo
			}
			else {
				$this->set_error(sprintf($this->_messages['is_equals'], $this->_data['name'], $campo));//nao exibe o valor e sim a string informada
			}
        }
        return $this;
    }
     ...
     * @return Data_Validator The self instance
     */
    public function is_not_equals($value, $identical = false, $campo = ""){
        $verify = ($identical === true ? $this->_data['value'] !== $value : strtolower($this->_data['value']) != strtolower($value));
        if(!$verify){
			if($campo == ""){
				$this->set_error(sprintf($this->_messages['is_not_equals'], $this->_data['name'], $value));//mostra o valor do campo
			}
			else {
				$this->set_error(sprintf($this->_messages['is_not_equals'], $this->_data['name'], $campo));//nao exibe o valor e sim a string informada
			}
        }
        return $this;
    }
